...
bot.inhibitors.set("permlevel", async function (data, command) {
  ...
  if (typeof command.permissionLevels === "function") {
    ...
    const allowed = await command.permissionLevels(data, command);
  ...
  for (const permlevel of command.permissionLevels) {
    ...
    const allowed = await hasPermission(data, command);
