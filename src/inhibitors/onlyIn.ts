...
bot.inhibitors.set("onlyIn", function (data, command) {
  // If the command is guildOnly and does not have a guild, inhibit the command
  if ((command.guildOnly || command.guildOnly === undefined) && !data.guildId) return true;
  // If the command is dmOnly and there is a guild, inhibit the command
  if (command.dmOnly && data.guildId) return true;
